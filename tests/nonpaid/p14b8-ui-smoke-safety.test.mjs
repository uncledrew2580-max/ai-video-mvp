...
test('workflow env defaults to image-only scope (video disabled unless explicitly authorized)', () => {
  ...
  assert.ok(/REAL_SMOKE_SCOPE:\s*\$\{\{[\s\S]{0,160}inputs\.smoke_scope \|\| 'image_only'/.test(yml), 'REAL_SMOKE_SCOPE defaults image_only');
...
test('B8R1: image-only default + dispatch-only trigger are preserved', () => {
  const yml = readWf();
  assert.ok(/REAL_SMOKE_SCOPE:\s*\$\{\{[\s\S]{0,160}inputs\.smoke_scope \|\| 'image_only'/.test(yml), 'image-only default preserved');
