...
/**
 * Run a sqlite query/script. node:sqlite is AUTHORITATIVE when available; the
 * sqlite3 CLI is used ONLY when node:sqlite is unavailable (old runtime).
 *
 * P14-B8G: we must NOT fall back to the CLI on a SQL error. node:sqlite enforces
 * foreign keys by default while the sqlite3 CLI has them OFF — re-running a failed
 * script through the CLI would mask a real constraint failure and persist bad
 * rows (and skip the transaction rollback). Surfacing the node:sqlite error is the
 * correct behavior, and matches the Windows portable (which ships no sqlite3 CLI).
 */
